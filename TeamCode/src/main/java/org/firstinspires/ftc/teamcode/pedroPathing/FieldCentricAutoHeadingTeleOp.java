...
public class FieldCentricAutoHeadingTeleOp extends OpMode {
    ...
    @Configurable public static double OMEGA_MAX_RAD = Math.toRadians(180); // max yaw rate deg/s -> rad/s
    @Configurable public static double KP_HOLD       = 3.0;                 // rad/s per rad error for heading hold
    @Configurable public static double L_CONV_M      = 1.524;               // 5 ft in meters, convergence distance for auto-heading
    @Configurable public static double SLEW_RATE_UNITS_PER_S = 3.0;         // left-stick rate limit (0..1 input units per second)
    @Configurable public static double DIR_RESET_DEG = 11;                  // threshold in degrees for desired heading change to reset convergence distance
